      ...
      {sessionState === "active" && interview && (
        <div className={`flex flex-col items-center gap-5 w-full ${showSimulator ? "max-w-5xl" : "max-w-3xl"}`}>
          ...
          <div className={`flex gap-4 w-full ${showSimulator ? "flex-row" : "flex-col items-center"}`}>
            {showSimulator && (
              <div
                className="relative rounded-2xl overflow-hidden border border-white/[0.08] flex-1"
              ...
              >
                ...
                />
                <div
                  className="absolute bottom-3 left-3 px-2 py-0.5 rounded text-xs text-white/30"
            ...
            <div
              className={`relative rounded-2xl overflow-hidden border border-white/[0.08] ${showSimulator ? "flex-1" : ""}`}
              style={{ boxShadow: "0 0 0 1px rgba(255,255,255,0.02), 0 24px 64px rgba(0,0,0,0.6)" }}
            >
              <video ref={videoRef} autoPlay playsInline className={`object-cover block ${showSimulator ? "w-full h-[360px]" : "w-[640px] h-[480px]"}`} />
              <canvas ref={canvasRef} className={`absolute top-0 left-0 ${showSimulator ? "w-full h-[360px]" : "w-[640px] h-[480px]"}`} />
              <button
                onClick={toggleDots}
